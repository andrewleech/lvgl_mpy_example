...
class driver:
    ...
    def init_gui_STM32F429_disco(self):
        from machine import SPI, Pin, I2C
        from ili9xxx import Ili9341
        import stmpe811

        print('Using STM32F429 Disco GUI')
        spi = SPI(5, baudrate=24_000_000)
        lcd_cs = Pin("C2", Pin.OUT)
        lcd_dc = Pin("D13", Pin.OUT)

        self.disp = Ili9341(spi=spi, cs=lcd_cs, dc=lcd_dc, rst=None)
        self.disp.set_backlight(100)

        # Register touch sensor
        i2c = I2C(3)
        assert "scl=A8" in str(i2c) and "sda=C9" in str(i2c)
        i2c_address = 0x82 >> 1
        stmpe811.init(i2c, i2c_address)
        indev_drv = lv.indev_drv_t()
        indev_drv.init()
        indev_drv.type = lv.INDEV_TYPE.POINTER
        indev_drv.read_cb = stmpe811.ts_read
        indev_drv.register()
